...
具体到每一天，看总共有几个状态，再找出每种状态对应的选择，然后根据对应的选择更新状态。
状态有3个：第几天、允许的最大交易次数、当前的持有状态(这是个需要挖掘的隐含状态，就像打家劫舍问题中每间房子偷/不偷)

k表示允许的最多交易次数。状态转移的思想是：交易k次可以从交易k-1次的结果转换而来，k次的结果包含了k-1次的结果
...
因此对dp[i][k]的定义需要分成两项：
dp[i][k][0]表示第i天结束时，进行了k笔交易且交易完成后，我们当前不持股，而获得的利润；
dp[i][k][1]表示第i天结束时，进行了k笔交易且交易完成后，我们当前持股，而获得的利润。
...
注意：由于交易包含两个成对出现的操作（买入、卖出），因此卖出方程中 允许交易的最大数量k保持不变，
...
注意 允许的最大交易次数 减少了一次，因为每次买入操作会使用一次交易
...
dp[i][1][1] = max(dp[i-1][1][1], dp[i-1][0][0] - prices[i]) 
            ...
            # 注意，状态0指：卖出股票后不持有股票的状态
            # 这和 没有进行过任何一次交易的不持有股票的状态 的区别
            # 因为k=1，即只允许一次交易，所以不能加上 dp[i-1][0][0]
'''
...
def maxProfit(k: int, prices: List[int]) -> int:
    ...
    dp[0][0] = 0
    dp[0][1] = -prices[0]  # 如果第1天就买入，那自然是“亏损”了
    for i in range(1, n):
