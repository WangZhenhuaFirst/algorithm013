...
二、DP，O(N^2)
为了从一个较短的上升子序列 得到一个较长的上升子序列，我们主要关心这个较短的上升子序列 结尾的元素。
由于要保证子序列的相对顺序，在程序读到一个新的数的时候，如果比已经得到的子序列的最后一个数 还大，
...
遍历到一个新数的时候，之前所有的状态值 都得保留，因此无法压缩。
...
def lengthOfLIS(self, nums: List[int]) -> int:
    ...
    '''
    n = len(nums)
    # 特殊处理必须要做，如果不做，当n=0，dp也就是空的，return max(dp)就会报错
    if n < 2:
        return n
    dp = [1] * n  # 至少是1
    for i in range(1, n):
