...
由于要考虑空字符，因此状态空间 要多设置一行、多设置一列。
base case：从一个 非空字符串 变成 空字符串，非空字符串的长度就是编辑距离；

思考状态的方法：
1、题目问什么就将什么定义为状态；
2、状态转移方程 怎么好推导，就怎么定义状态；
3、根据经验和问题的特点（只有多做题了）
dp[i][j]表示 word1 到i位置，转换成 word2 到j位置 需要的最少步数


状态转移方程 通常是在做 分类讨论，而分类讨论的过程，常常利用了这个问题的「最优子结构」。
当word1[i] == word2[j], dp[i][j] = dp[i-1][j-1]
当word1[i] != word2[j], dp[i][j] = min(dp[i-1][j-1], dp[i-1][j], dp[i][j-1]) + 1
dp[i-1][j-1]到dp[i][j]需要进行 替换操作，dp[i-1][j]到dp[i][j]需要进行删除操作，
...
def minDistance(self, word1: str, word2: str) -> int:
    ...
    for i in range(1, n1 + 1):  # 注意，这里都要+1，
        dp[i][0] = i
    for j in range(1, n2 + 1):
        dp[0][j] = j
    for i in range(1, n1+1):
        for j in range(1, n2+1):
            if word1[i-1] == word2[j-1]:  # 注意，是 i - 1
                dp[i][j] = dp[i-1][j-1]
            else:
