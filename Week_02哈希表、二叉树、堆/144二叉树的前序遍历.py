...
为什么前中后序遍历 迭代法不统一？
因为迭代的过程中，我们有两个操作:处理，即将元素放进result中；访问，即遍历这一节点。
前序是 根左右，先访问的元素是中间节点，要处理的也是中间节点，所以才能写出相对简洁的代码。
中序是 左根右，先访问的是二叉树根节点，然后一层层向下访问，直到树左面的最底部，再开始处理节点。
也就是处理顺序和访问顺序 是不一致的。 所以，用迭代法写中序遍历，就需要借用指针的遍历 来帮助访问节点，
栈则用来处理节点上的元素。
怎么写出前中后序遍历 迭代法统一的写法？
将访问的节点 放入栈中
将要处理的节点 也放入栈中，但要做标记。也就是要处理的节点 放入栈之后，紧接着放入一个空指针作为标记。
